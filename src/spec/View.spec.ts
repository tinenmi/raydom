import { $, I, Tracker, View, cast, lense } from "../lib/index"
import { ray } from "../lib/helpers/ray"
...
describe('View', () => {
  ...
  it('view can be created as cast', async () => {
    ...
    let $value = I($sourceValue, doubler)
    $sourceValue(2)
    let value = $value()
    expect(value).toBe(4)
  })
  ...
  it('view can be created as lense', async () => {
    ...
    let $value = I($rootValue, lense('prop'))
    $rootValue({ prop: 2 })
    let value = $value()
    expect(value).toBe(2)
  })
  ...
  it('view can be created as ray template string', async () => {
    ...
    $data(2)
    let view = $view()
    expect(view).toBe('View has data: 2')
  })
  ...
  it('chain of views', async () => {
    ...
    expect($model4()).toBe(1)
  })

  it('counter', async () => {
    let $counter = $.new(0)
    let $isEven = I($counter, cast((x: any) => (x & 1) == 0))
    let $parity = I($isEven, cast((x: any) => x ? "even" : "odd"))
